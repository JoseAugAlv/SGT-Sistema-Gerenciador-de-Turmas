...
$prazoAtual = '';
if (!empty($criterio['prazo_avaliacao'])) {
    $dt = new DateTime($criterio['prazo_avaliacao']);
    $prazoAtual = $dt->format('Y-m-d\TH:i');
}
?>
...
<form method="POST" action="<?= $basePath ?>/criterios/<?= (int) $criterio['id'] ?>/atualizar">
    <?= ViewHelper::csrfField() ?>

    <p><label>Nome *<br>
        <input type="text" name="nome" maxlength="200" value="<?= h($criterio['nome']) ?>" required></label></p>

    <p><label>Descrição<br>
        <textarea name="descricao" rows="3" maxlength="2000"><?= h($criterio['descricao'] ?? '') ?></textarea></label></p>

    <p><label>Peso *<br>
        <input type="number" name="peso" min="0.01" max="99.99" step="0.01"
               value="<?= number_format((float) $criterio['peso'], 2, '.', '') ?>" required></label></p>

    <p><label>Tipo de avaliação *<br>
        <select name="tipo_avaliacao" required>
            <?php foreach ($tipos as $val => $label): ?>
                <option value="<?= h($val) ?>" <?= $val === $criterio['tipo_avaliacao'] ? 'selected' : '' ?>>
                    <?= h($label) ?>
                </option>
            <?php endforeach; ?>
        </select></label></p>
    ...
    <p><label>Aplicável a *<br>
        <select name="aplicavel_a" required>
            <?php foreach ($aplicaveis as $val => $label): ?>
                <option value="<?= $val ?>" <?= $val === $criterio['aplicavel_a'] ? 'selected' : '' ?>>
                    <?= h($label) ?>
                </option>
            <?php endforeach; ?>
        </select></label></p>
            ...
            <select name="etapa_id">
                ...
                <?php foreach ($etapas as $e): ?>
                    <option value="<?= (int) $e['id'] ?>"
                        <?= (int) $criterio['etapa_id'] === (int) $e['id'] ? 'selected' : '' ?>>
                        <?= h($e['nome']) ?>
                    </option>
                <?php endforeach; ?>
            </select></label></p>
    ...
    <p><label>Prazo de avaliação (opcional)<br>
        <input type="datetime-local" name="prazo_avaliacao" value="<?= h($prazoAtual) ?>"></label></p>
