...
<h2>Boletim — <?= h($aluno['nome']) ?></h2>

<?php if (!empty($boletim['congelado'])): ?>
    <p>
        <i class="fas fa-lock" style="color:#b00"></i>
        <strong>Boletim congelado</strong> em <?= h($boletim['congelado_em']) ?>.
        Este projeto foi encerrado — os valores abaixo não mudam mais.
    </p>
<?php endif; ?>

<?php if (empty($boletim['linhas'])): ?>
    <p>Nenhum critério definido neste projeto.</p>
<?php else: ?>
...
<p>
    <a href="<?= $basePath ?>/projetos/<?= (int) $projeto['id'] ?>/relatorios/boletim/pdf?aluno_id=<?= (int) $aluno['id'] ?>">
        <i class="fas fa-file-pdf"></i> Exportar PDF
    </a>
</p>

<p>
    <a href="<?= $basePath ?>/projetos/<?= (int) $projeto['id'] ?>/relatorios">
